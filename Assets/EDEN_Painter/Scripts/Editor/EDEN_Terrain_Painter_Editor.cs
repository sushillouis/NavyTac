...
namespace GapperGames
{
    ...
    public class EDEN_Terrain_Painter_Editor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            Terrain terrain = eden.GetComponent<Terrain>();

            if (GUILayout.Button("Paint Terrain"))
            {
                if (terrain == null)
                {
                    EditorGUILayout.HelpBox("EDEN Terrain Painter requires a Terrain component on the same GameObject.", MessageType.Error);
                    return;
                }
